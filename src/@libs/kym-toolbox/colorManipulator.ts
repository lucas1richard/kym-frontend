...
export function rgbToHex(color: string | string[]) {
  ...
  function intToHex(c: { toString: (arg0: number) => any; }) {
 ...
 */
export function recomposeColor(color: { type: string; values: (number | string)[] }) {
  const { type } = color;
  let { values } = color;
  ...
  if (type.indexOf('rgb') !== -1) {
    ...
    // @ts-ignore
    values = values.map((n: string | number, i: number) => (i < 3 ? parseInt(n, 10) : n));
  }
